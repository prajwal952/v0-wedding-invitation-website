...
export function CountdownTimer() {
  ...
  return (
    <section className="py-12 sm:py-16 bg-gradient-to-b from-secondary/10 via-background to-primary/5">
      <div className="container mx-auto px-4">
        <h2 className="text-center font-serif text-3xl sm:text-4xl md:text-5xl text-primary mb-4">
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap justify-center gap-3 sm:gap-4 md:gap-8 max-w-md sm:max-w-none mx-auto">
          {timeUnits.map((unit) => (
            <div
              key={unit.label}
              className="relative group"
            >
              <div className="w-full h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 bg-card rounded-lg border-2 border-secondary/50 shadow-lg flex flex-col items-center justify-center transition-all duration-300 group-hover:border-secondary group-hover:shadow-xl group-hover:scale-105">
                <span className="text-2xl sm:text-3xl md:text-5xl font-bold text-primary">
                  {unit.value.toString().padStart(2, "0")}
                </span>
                <span className="text-xs uppercase tracking-wider sm:tracking-widest text-muted-foreground mt-1">
                  {unit.label}
                </span>
              </div>
              {/* Decorative corner */}
